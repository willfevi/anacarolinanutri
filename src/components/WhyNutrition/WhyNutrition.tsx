export default function WhyNutrition() {
  return (
    <section id="por-que-nutricao" className="px-8 py-20 bg-[#fff] border-t border-[#efe7ec]">
      <p className="italic text-[#7f0e48] mb-4 text-center">Por que a nutrição é essencial?</p>

      <h2 className="font-['Playfair_Display'] text-3xl text-[#3a2230] max-w-2xl mx-auto leading-relaxed mb-6 text-center">De uma hora para outra, tudo muda.</h2>

      <p className="text-[#6b4c5d] max-w-3xl mx-auto mb-8 text-center">De uma hora para outra, tudo muda. O paladar altera, o apetite some e surgem medos sobre o que pode ou não comer. Passar por tudo isso sozinha torna o caminho muito mais pesado. O acompanhamento certo é o que ajuda seu corpo a se sustentar e as escolhas do dia a dia a fazerem sentido.</p>

      <div className="max-w-3xl mx-auto text-center text-[#6b4c5d]">
        <h3 className="font-['Playfair_Display'] text-xl md:text-2xl text-[#3a2230] mb-4">Onde a nutrição faz a diferença:</h3>

        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl p-5 shadow-md border-l-4 border-[#7f0e48]">
            <div>
              <div className="font-semibold text-[#7f0e48]">Alterações Digestivas:</div>
              <div className="text-[#6b4c5d]">Náuseas, vômitos, diarreia ou constipação.</div>
            </div>
          </div>

          <div className="bg-white rounded-2xl p-5 shadow-md border-l-4 border-[#7f0e48]">
            <div>
              <div className="font-semibold text-[#7f0e48]">Impacto no Apetite e Paladar:</div>
              <div className="text-[#6b4c5d]">Falta de apetite, feridas na boca (mucosite), boca seca e alterações na percepção do sabor dos alimentos.</div>
            </div>
          </div>

          <div className="bg-white rounded-2xl p-5 shadow-md border-l-4 border-[#7f0e48]">
            <div>
              <div className="font-semibold text-[#7f0e48]">Sintomas Metabólicos e Hormonais:</div>
              <div className="text-[#6b4c5d]">Fadiga extrema, fogachos (calores), ganho de peso indesejado ou perda de peso acelerada.</div>
            </div>
          </div>

          <div className="bg-white rounded-2xl p-5 shadow-md border-l-4 border-[#7f0e48]">
            <div>
              <div className="font-semibold text-[#7f0e48]">Composição Corporal:</div>
              <div className="text-[#6b4c5d]">Perda de massa muscular (sarcopenia) e fraqueza física.</div>
            </div>
          </div>

          <div className="bg-white rounded-2xl p-5 shadow-md border-l-4 border-[#7f0e48]">
            <div>
              <div className="font-semibold text-[#7f0e48]">Dificuldades de Ingestão:</div>
              <div className="text-[#6b4c5d]">Dor ou dificuldade para engolir e saciedade precoce.</div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
